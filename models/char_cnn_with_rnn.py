...
def write_words_to_file(orig_words, predictions):
	...
	filename = "./outputs/RCNN_globalavgPool/multitask_context_out.txt"
...
def write_features_to_file(orig_features, pred_features, encoders):
	...
	for i in range(len(orig_features)):
		filename = "./outputs/RCNN_globalavgPool/feature"+str(i)+"context_out.txt"
		with open(filename, 'w', encoding='utf-8') as f:
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6,
	...
	x = Dense(HIDDEN_DIM, activation='relu', kernel_initializer='he_normal',
			  kernel_constraint= maxnorm(3), bias_constraint=maxnorm(3), name='mlp_common')(x)

	x = Dropout(0.1, name='drop')(x)

	out1 = Dense(n1, init='he_normal', activation='softmax', name='output1')(x)
	out2 = Dense(n2, init='he_normal', activation='softmax', name='output2')(x)
	out3 = Dense(n3, init='he_normal', activation='softmax', name='output3')(x)
	out4 = Dense(n4, init='he_normal', activation='softmax', name='output4')(x)
	out5 = Dense(n5, init='he_normal', activation='softmax', name='output5')(x)
	out6 = Dense(n6, init='he_normal', activation='softmax', name='output6')(x)
	...
	BidireLSTM_curr = Bidirectional(GRU(40, dropout=dropout, return_sequences=False))(current_word_embedding)
	...
	for _ in range(num_layers):
		LtoR_LSTM = Bidirectional(GRU(40, dropout=dropout, return_sequences=True))
	...
	opt = Adam()
	model.compile(optimizer=Adadelta(lr=0.95, epsilon=1e-06), loss='categorical_crossentropy',
				  metrics=['accuracy'],
...
model = create_model(X_vocab_len, X_max_len, y_vocab_len, X_max_len,
					 y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y7, n7, HIDDEN_DIM, LAYER_NUM)

saved_weights = "./model_weights/multiTask_with_charCRNN_globalPool.hdf5"
...
if MODE == 'train':
	...
	hist = model.fit([X, X_left1, X_left2, X_left3, X_left4, X_right1, X_right2, X_right3, X_right4],
					 [y_sequences, y1, y2, y3, y4, y5, y7],
					 validation_split=0.25,
					 batch_size=BATCH_SIZE, epochs=EPOCHS,
					 callbacks=[EarlyStopping(patience=7),
								ModelCheckpoint('./model_weights/multiTask_with_charCRNN_globalPool.hdf5', save_best_only=True,
												verbose=1)])
	...
	if len(saved_weights) == 0:
		...
		print(model.summary())
		print(model.evaluate([X_test, X_left1, X_left2, X_left3, X_left4, X_right1, X_right2, X_right3, X_right4],
							 [y_test_seq, y1, y2, y3, y4, y5, y7]))
		print(model.metrics_names)

		words, f1, f2, f3, f4, f5, f7 = model.predict(
			[X_test, X_left1, X_left2, X_left3, X_left4, X_right1, X_right2, X_right3, X_right4])
		...
		if output_mode == 'dump':
			...
			pickle.dump(pred_features, open('./pickle-dumps/predictions_rcnn_avgpool', 'wb'))
