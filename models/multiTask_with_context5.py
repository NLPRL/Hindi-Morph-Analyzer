...
def write_words_to_file(orig_words, predictions):
	...
	filename = "./outputs/BLSTM_with_context5/multitask_context1_out.txt"
...
def write_features_to_file(orig_features, pred_features, encoders):
	...
	for i in range(len(orig_features)):
		filename = "./outputs/BLSTM_with_context5/feature"+str(i)+"context2_out.txt"
		with open(filename, 'w', encoding='utf-8') as f:
...
def create_model(X_vocab_len, X_max_len, y_vocab_len, y_max_len, y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6, hidden_size, num_layers):
	...
	right_word5 = Input(shape=(X_max_len,), dtype='int32')
	...
	left_word5 = Input(shape=(X_max_len,), dtype='int32')
	...
	right_word_embedding5 = emb_layer(right_word5)
	...
	left_word_embedding5 = emb_layer(left_word5)
	...
	BidireLSTM_right4 = Bidirectional(LSTM(40, dropout=dropout, return_sequences=False))(right_word_embedding4)
	BidireLSTM_right5 = Bidirectional(LSTM(40, dropout=dropout, return_sequences=False))(right_word_embedding5)
	...
	BidireLSTM_left4 = Bidirectional(LSTM(40, dropout=dropout, return_sequences=False))(left_word_embedding4)
	BidireLSTM_left5 = Bidirectional(LSTM(40, dropout=dropout, return_sequences=False))(left_word_embedding5)
	...
	concatenated_encodings.append(BidireLSTM_left5)
	concatenated_encodings.append(BidireLSTM_right5)
	...
	all_inputs = [current_word, right_word1, right_word2, right_word3, right_word4, right_word5, left_word1, left_word2, left_word3, left_word4, left_word5]
...
X, X_vocab_len, X_word_to_ix, X_ix_to_word, y, y_vocab_len, y_word_to_ix, y_ix_to_word, X_left1, X_left2, X_left3, X_left4, X_left5, X_right1, X_right2,  X_right3, X_right4, X_right5 = \
	load_data_for_seq2seq(sentences, rootwords, test=False, context5=True)
...
X_left5 = pad_sequences(X_left5, maxlen = X_max_len, dtype='int32', padding='post')
...
X_right5 = pad_sequences(X_right5, maxlen = X_max_len, dtype='int32', padding='post')
...
saved_weights = "./model_weights/multiTask_with_context5.hdf5"

if MODE == 'train':
	print("Training model ..")
	y_sequences = process_data(y, y_max_len, y_word_to_ix)

	hist = model.fit([X, X_left1, X_left2, X_left3, X_left4, X_left5, X_right1, X_right2, X_right3, X_right4, X_right5], [y_sequences, y1, y2, y3, y4, y5, y7], 
		validation_split=0.25, 
		batch_size=BATCH_SIZE, epochs=EPOCHS,  
		callbacks=[EarlyStopping(patience=7),
		ModelCheckpoint('./model_weights/multiTask_with_context5.hdf5', save_best_only=True,
			verbose=1)])
	...
	if len(saved_weights) == 0:
		...
		X_left1, X_left2, X_left3, X_left4, X_left5, X_right1, X_right2, X_right3, X_right4, X_right5 = \
				load_data_for_seq2seq(test_sentences, test_roots, features, labels, test=True, context5=True)
		...
		plot_model(model, to_file="multi_task_attEnc_arch_with_context5.png", show_shapes=True)

		print(model.evaluate([X_test, X_left1, X_left2, X_left3, X_left4, X_left5, X_right1, X_right2, X_right3, X_right4, X_right5], [y_test_seq, y1, y2, y3, y4, y5, y7]))
		print(model.metrics_names)
		
		words, f1, f2, f3, f4, f5, f7 = model.predict([X_test, X_left1, X_left2, X_left3, X_left4, X_left5, X_right1, X_right2, X_right3, X_right4, X_right5])
		...
		pickle.dump(pred_features, open('./pickle-dumps/predictions_context5', 'wb'))
