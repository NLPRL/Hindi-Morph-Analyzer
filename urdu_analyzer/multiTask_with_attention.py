...
def write_words_to_file(orig_words, predictions):
	...
	# print(sentences[:10])
	sentences = pickle.load(open('./pickle_dumps/sentences_test', 'rb'))
...
def process_features(y1,y2,y3,y4,y5,y6, n = None, enc=None):
	...
	if enc == None:
		...
		for i in range(len(y)):
			arr = [w if w in list(enc[i].classes_) else 'uncertain' for w in y[i]]
...
def create_model(X_vocab_len, X_max_len, y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6, hidden_size, num_layers):
	...
	x = Dense(HIDDEN_DIM, activation='relu', kernel_initializer='he_normal',
			  kernel_constraint= maxnorm(3), bias_constraint=maxnorm(3), name='dense1')(x)
	...
	encoder_forward = GRU(rnn_output_size, return_sequences=True, unroll=True, name='encoder_f')(current_word_embedding)
	encoder_forward_last = encoder_forward[:,-1,:]

	encoder_backward = GRU(rnn_output_size, return_sequences=True, unroll=True, go_backwards=True, name='encoder_b')(current_word_embedding)
	...
	decoder_forward = GRU(rnn_output_size, return_sequences=True, unroll=True, name='decoder_f')(decoder, initial_state=[encoder_forward_last])
	decoder_backward = GRU(rnn_output_size, return_sequences=True, unroll=True, go_backwards=True, name='decoder_b')(decoder, initial_state=[encoder_backward_last])
	...
	all_inputs = [current_word, decoder_input, right_word1, right_word2, right_word3, right_word4, left_word1, left_word2, left_word3, left_word4]
...
pseudo_model = create_model(X_vocab_len, X_max_len, 
					 y1, n1, y2, n2, y3, n3, y4, n4, y5, n5, y6, n6, HIDDEN_DIM, LAYER_NUM)
...
if MODE == 'train':
	...
	hist = model.fit([X_train, X_decoder_input,  X_right1_tr, X_right2_tr, X_right3_tr, X_right4_tr, X_left1_tr, X_left2_tr, X_left3_tr, X_left4_tr],
				 [y_sequences_tr, y1_tr, y2_tr, y3_tr, y4_tr, y5_tr, y6_tr],
				 validation_data=([X_val, X_decoder_val,  X_right1_val, X_right2_val, X_right3_val, X_right4_val, X_left1_val, X_left2_val, X_left3_val, X_left4_val],\
				 [y_sequences_val, y1_val, y2_val, y3_val, y4_val, y5_val, y6_val]),
	...
	names_to_be_freezed = ['Embedding', 'merge_1', 'drop1_0', 'drop1_1', 'drop1_2', 'drop1_3', 'drop1_4', 'drop1_5', 'drop1_6', 'drop1_7', 
		...
		'bidirec1', 'dense1', 'drop_single2', 'dense2',  'drop_single3', 'output1', 'output2', 'output3', 'output4', 
		'output5', 'output6']
	...
	hist = pseudo_model.fit([X_train, X_decoder_input,  X_right1_tr, X_right2_tr, X_right3_tr, X_right4_tr, X_left1_tr, X_left2_tr, X_left3_tr, X_left4_tr],
				 [y_sequences_tr, y1_tr, y2_tr, y3_tr, y4_tr, y5_tr, y6_tr],
				 validation_data=([X_val, X_decoder_val,  X_right1_val, X_right2_val, X_right3_val, X_right4_val, X_left1_val, X_left2_val, X_left3_val, X_left4_val],\
				 [y_sequences_val, y1_val, y2_val, y3_val, y4_val, y5_val, y6_val]),
	...
	if len(saved_weights) == 0:
		...
		test_sentences = pickle.load(open('./pickle_dumps/sentences_test', 'rb'))
		test_roots = pickle.load(open('./pickle_dumps/rootwords_test', 'rb'))
		test_features = pickle.load(open('./pickle_dumps/features_test', 'rb'))
