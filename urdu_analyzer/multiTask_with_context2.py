...
def write_words_to_file(orig_words, predictions):
	...
	filename = "./outputs/BLSTM_context2_and_attention_new/multitask_context1_out.txt"
...
def write_features_to_file(orig_features, pred_features, encoders):
	...
	for i in range(len(orig_features)):
		filename = "./outputs/BLSTM_context2_and_attention_new/feature"+str(i)+"context2_out.txt"
		with open(filename, 'w', encoding='utf-8') as f:
...
if MODE == 'train':
	...
	if len(saved_weights) == 0:
		...
		pickle.dump(pred_features, open('./pickle-dumps/predictions_context2_with_att_new', 'wb'))
